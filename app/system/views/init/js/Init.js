...
//                            ]}
                ...
                {name: 'footer', content: '<div class="mui-copyright">&copy; 2014 <a href="#" id="authorsLink">Livian Framework</a> - <a href="#" id="licenseLink">DC</a></div>', cssClass: 'mui-desktopFooter'}
            ],
            'onDrawEnd': function () {
...
//				}
                ...
                onDragStart: function (instance) {
                    if (!Browser.ie)
                        instance.el.windowEl.setStyle('opacity', 0.5);
                    // VML doesn't render opacity nicely on the shadow
                },
                onDragComplete: function (instance) {
                    if (!Browser.ie)
                        instance.el.windowEl.setStyle('opacity', 1);
                },
                onLoaded: function () {
        ...
        createModal: function () {
            ...
            if (Demo.modalCount < 3)
                content += '<br/><br/><a id="createModal' + Demo.modalCount + '">Create Another Modal</a>';
            MUI.create({
                control: 'MUI.Modal',
                ...
                onLoaded: function () {
                    if ($('createModal' + Demo.modalCount))
                        $('createModal' + Demo.modalCount).addEvent('click', function (e) {
                            e.stop();
                            Demo.createModal();
                        });
                }
        ...
        addResizeElements: function () {		// add resize events to panels
            ...
            if (!resize)
                resize = new Element('div', {'class': 'resizeInfo'}).inject(pad);
            resize.empty();
        ...
        updateResizeElements: function () {	// update the resize counts for panels
            ...
            if (this.countEvents)
                this.countEvents.set('text', Demo.countResizeEvents[this.id]);
            var newSize = this.el.contentWrapper.getStyles(['width', 'height']);
            if (this.displayWidthEl)
                this.displayWidthEl.set('text', newSize['width']);
            if (this.displayHeightEl)
                this.displayHeightEl.set('text', newSize['height']);
        }
    });
    /*=============================================*/
